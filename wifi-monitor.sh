...
if [ "$#" -ne 2 ]; then
    echo "Usage: $0 <network_location> <log_file>"
    exit 1
fi
...
INITIAL_DEVICES=$(arp-scan --interface=eth0 --localnet | grep -o -E '([[:xdigit:]]{1,2}:){5}[[:xdigit:]]{1,2}')
...
while true; do
    ...
    CURRENT_DEVICES=$(arp-scan --interface=eth0 --localnet)
    ...
    if [ ! -z "$NEW_MACS" ]; then
        ...
        for MAC in $NEW_MACS; do
            ...
            MANUFACTURER=$(arp-scan --interface=eth0 --localnet | grep "$MAC" | awk -F'\t' '{print $3}')
            TIMESTAMP=$(date)
